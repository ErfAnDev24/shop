...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  void initState() {
    BlocProvider.of<ProductDetailsBloc>(context).add(
        RequestProductDetailsEvent('at0y1gm0t65j62j', widget.product.category));
    super.initState();
  }
            ...
            BlocBuilder<ProductDetailsBloc, ProductDetailsState>(
              builder: (context, state) {
                if (state is LoadingProductDetailsState) {
                  ...
                  return state.productVariantList.fold(
                      (left) => SliverToBoxAdapter(
                            child: Text(left),
                          ), (right) {
                    return SliverToBoxAdapter(
                      child:
                          VariantComponentGenerator(productVariantList: right),
                    );
                  });
                }
                return const SliverToBoxAdapter(
